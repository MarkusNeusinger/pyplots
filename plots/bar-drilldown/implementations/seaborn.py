...
# Set seaborn style for cleaner appearance
...
# Define distinct, high-contrast color palettes
level1_colors = ["#2E86AB", "#F6AE2D", "#A23B72"]  # Blue, Gold, Magenta
level2_colors = ["#1B998B", "#E55934", "#7B2CBF"]  # Teal, Orange-Red, Purple
level3_colors = ["#2E86AB", "#06D6A0", "#FF6B6B"]  # Blue, Green, Coral
...
for i, (_, row) in enumerate(root_df.iterrows()):
    axes[0].text(i, row["value"] + 25, f"${row['value']}M", ha="center", va="bottom", fontsize=14, fontweight="bold")
    # Add click indicator arrow inside bar
    axes[0].annotate(
        "\u25bc", xy=(i, row["value"] * 0.15), ha="center", va="center", fontsize=16, color="white", fontweight="bold"
    )

axes[0].set_xlabel("Region", fontsize=18)
axes[0].set_ylabel("Sales ($ Millions)", fontsize=18)
axes[0].set_title("Level 1: All Regions\n\u2193 Click bar to drill down", fontsize=16, fontweight="bold", color="#333")
...
for i, (_, row) in enumerate(na_children.iterrows()):
    axes[1].text(i, row["value"] + 15, f"${row['value']}M", ha="center", va="bottom", fontsize=14, fontweight="bold")
    axes[1].annotate(
        "\u25bc", xy=(i, row["value"] * 0.15), ha="center", va="center", fontsize=16, color="white", fontweight="bold"
    )
...
for i, (_, row) in enumerate(usa_children.iterrows()):
    axes[2].text(i, row["value"] + 8, f"${row['value']}M", ha="center", va="bottom", fontsize=14, fontweight="bold")

axes[2].set_xlabel("City", fontsize=18)
axes[2].set_ylabel("Sales ($ Millions)", fontsize=18)
axes[2].set_title(
    "Level 3: All \u203a NA \u203a USA\n\u2714 Deepest level reached", fontsize=16, fontweight="bold", color="#333"
)
axes[2].tick_params(axis="both", labelsize=14)
axes[2].set_ylim(0, 260)

# Main title
fig.suptitle("bar-drilldown \u00b7 seaborn \u00b7 pyplots.ai", fontsize=24, fontweight="bold", y=1.02)
