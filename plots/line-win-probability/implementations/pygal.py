...
# Fill data: Cowboys constant at 50%, Eagles at actual probability
cowboys_fill = [50.0] * len(win_pct_list)
...
custom_style = Style(
    ...
    colors=("#003594", "#004C54", "#111111", "#d4380d"),
...
eagles_data = [
    {"value": pct, "label": scoring_events[i]} if i in scoring_events else pct for i, pct in enumerate(win_pct_list)
]
chart.add("PHI Eagles", eagles_data, fill=True, stroke_style={"width": 5})

# Series 3: 50% baseline reference line (prominent, rendered on top of fills)
baseline = [50] * len(win_pct_list)
chart.add("50% Line", baseline, fill=False, show_dots=False, stroke_style={"width": 9, "dasharray": "20, 8"})

# Series 4: Scoring event markers with tooltip labels
...
# X-axis labels: only quarter markers (clean, no crowding)
quarter_plays = {0: "Kickoff", 30: "Q2", 60: "Halftime", 90: "Q4", 120: "Final"}
chart.x_labels = [quarter_plays.get(i, "") for i in range(plays + 1)]
chart.x_labels_major = ["Kickoff", "Q2", "Halftime", "Q4", "Final"]
