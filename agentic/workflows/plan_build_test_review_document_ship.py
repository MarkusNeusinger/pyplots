...
# ///
"""
Complete pipeline orchestrator: Plan + Build + Test + Review + Document + Ship.

Chains all workflow phases into a single end-to-end pipeline.
Document failure is non-fatal. All other phase failures abort the pipeline.

Usage:
    uv run agentic/workflows/plan_build_test_review_document_ship.py "Add CSV export to API endpoints"
    uv run agentic/workflows/plan_build_test_review_document_ship.py "Fix the 404 bug" --type bug --model large
"""
...
def main(prompt: str, task_type: str, model: str, working_dir: str, cli: str):
    """Full pipeline: plan, build, test, review, document, and ship."""
    ...
    console.print(
        Panel(
            f"[bold blue]Full Pipeline Orchestrator[/bold blue]\n\n"
            f"[cyan]Pipeline:[/cyan] Plan \u2192 Build \u2192 Test \u2192 Review \u2192 Document \u2192 Ship\n"
            f"[cyan]Prompt:[/cyan] {prompt}\n"
            f"[cyan]Model:[/cyan] {model}\n"
            f"[cyan]CLI:[/cyan] {cli}",
            title="[bold blue]Plan + Build + Test + Review + Document + Ship[/bold blue]",
            border_style="blue",
        )
    )
    console.print()
    ...
    # ── Phase 6: Ship ─────────────────────────────────────────────
    console.print(Rule("[bold yellow]Phase 6: Ship (commit + PR)[/bold yellow]"))
    console.print()

    ship_args = ["--run-id", run_id] + common_args
    ship_rc, ship_stdout = run_phase("ship.py", ship_args, console, "Ship", capture_stdout=is_piped)

    if is_piped and ship_stdout:
        print(ship_stdout, end="")
