...
# Series 3: RSI-like indicator (bounded 0-100)
rsi = 50 + np.cumsum(np.random.normal(0, 5, n_points))
rsi = np.clip(rsi, 10, 90)

# Use categorical ordering for facets (Price at top, Volume middle, RSI bottom)
metric_order = pd.CategoricalDtype(categories=["Price ($)", "Volume (M)", "RSI Indicator"], ordered=True)
...
df = pd.DataFrame(
    {
        ...
        "metric": (["Price ($)"] * n_points + ["Volume (M)"] * n_points + ["RSI Indicator"] * n_points),
    }
)
...
annotation_df = pd.DataFrame(
    {
        ...
        "metric": ["Price ($)", "Volume (M)", "RSI Indicator"],
    }
)
...
label_df = pd.DataFrame(
    {
        "date": [crosshair_date] * 3,
        "value": [crosshair_price * 1.05, crosshair_volume * 1.5 + 0.5, crosshair_rsi + 8],
        "metric": ["Price ($)", "Volume (M)", "RSI Indicator"],
...
plot = (
    ...
    + geom_vline(xintercept=crosshair_date, color="#E74C3C", size=1, linetype="dashed", alpha=0.8)
    + geom_point(data=annotation_df, mapping=aes(x="date", y="value"), color="#E74C3C", size=5, alpha=0.9)
    + geom_label(
        data=label_df,
    ...
    + scale_y_continuous()
    + labs(title="dashboard-synchronized-crosshair · plotnine · pyplots.ai", x="Date", y="Value")
    + theme_minimal()
    + theme(
        figure_size=(16, 12),
