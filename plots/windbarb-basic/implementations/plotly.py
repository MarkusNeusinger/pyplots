...
def draw_wind_barb(x0, y0, u, v, barb_length=0.35, barb_width=0.12):
    """Generate line segments for a wind barb at position (x0, y0) with wind components (u, v).

    Returns lists of x and y coordinates for plotting lines.
    Half barb = 5 knots, full barb = 10 knots, pennant = 50 knots.
    Wind barbs point FROM which the wind blows.
    """
    ...
    if speed < 2.5:
        ...
        return list(cx) + [None], list(cy) + [None]

    # Calculate direction (wind comes FROM this direction)
    # Direction in radians where wind is coming FROM
    ...
    all_x = [x0, x_end, None]  # Staff line
    all_y = [y0, y_end, None]
    ...
    # Start near the end of the staff
    spacing = barb_length * 0.12
    current_pos = 0.95  # Start at 95% of staff length from base

    # Draw pennants (triangular flags for 50 knots)
    for _ in range(pennants):
        # Pennant base position along staff
        px1 = x0 + current_pos * barb_length * np.cos(wind_dir)
        py1 = y0 + current_pos * barb_length * np.sin(wind_dir)

        # Move down for pennant end
        current_pos -= 0.15
        px2 = x0 + current_pos * barb_length * np.cos(wind_dir)
        py2 = y0 + current_pos * barb_length * np.sin(wind_dir)

        # Pennant tip (perpendicular)
        px_tip = px1 + barb_width * np.cos(perp_dir)
        py_tip = py1 + barb_width * np.sin(perp_dir)

        # Draw filled triangle (as lines)
        all_x.extend([px1, px_tip, px2, px1, None])
        all_y.extend([py1, py_tip, py2, py1, None])

        current_pos -= 0.05  # Small gap after pennant
    ...
    for _ in range(full_barbs):
        ...
        all_x.extend([bx1, bx2, None])
        all_y.extend([by1, by2, None])
...
for i in range(len(X)):
    bx, by = draw_wind_barb(X[i], Y[i], U[i], V[i])
    all_barb_x.extend(bx)
    all_barb_y.extend(by)

# Add wind barbs as a single scatter trace with lines
fig.add_trace(
    go.Scatter(
        x=all_barb_x,
        y=all_barb_y,
...
fig.add_trace(
    go.Scatter(
        ...
        y=Y,
        mode="markers",
        marker={"size": 8, "color": "#306998", "symbol": "circle"},
...
fig.update_layout(
    ...
    showlegend=True,
    legend={
        "font": {"size": 18}, "x": 0.02, "y": 0.98, "bgcolor": "rgba(255,255,255,0.9)", "bordercolor": "#306998", "borderwidth": 1
    },
    margin={"l": 100, "r": 80, "t": 100, "b": 100},
    # Add annotation for wind barb legend
    annotations=[
        {
            "x": 0.98,
            "y": 0.02,
            "xref": "paper",
            "yref": "paper",
            "text": "Wind Barb Legend:<br>○ = Calm (&lt;2.5 kt)<br>╲ = 5 kt (half)<br>╲╲ = 10 kt (full)<br>▲ = 50 kt (pennant)",
            "showarrow": False,
