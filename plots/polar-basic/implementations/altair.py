...
# Normalize radius to positive values
radius = temperatures - temperatures.min() + 3
...
circles = (
    alt.Chart(grid_df)
    .mark_line(strokeWidth=1, opacity=0.3, color="#888888", strokeDash=[4, 4])
...
spokes = (
    alt.Chart(spoke_df)
    .mark_rule(strokeWidth=1, opacity=0.4, color="#888888")
...
labels = (
    alt.Chart(label_df)
    .mark_text(fontSize=20, fontWeight="bold", color="#333333")
    .encode(x="x:Q", y="y:Q", text="label:N")
)

# Data line connecting all points
line = (
    alt.Chart(df_path).mark_line(strokeWidth=4, color="#FFD43B", opacity=0.8).encode(x="x:Q", y="y:Q", order="order:O")
)

# Data points
points = (
    alt.Chart(df)
    .mark_point(filled=True, size=400, color="#306998", opacity=0.9)
    .encode(
        x="x:Q",
        y="y:Q",
        tooltip=[
...
chart = (
    ...
    .properties(
        width=1600,
        height=900,
        title=alt.Title(text="Hourly Temperature · polar-basic · altair · pyplots.ai", fontSize=28),
    )
