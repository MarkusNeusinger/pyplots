...
for dec_grid in range(-60, 90, 30):
    ...
    p.line(gx, gy, line_color=grid_color, line_alpha=grid_alpha, line_width=1, line_dash="dotted")

# RA lines
for ra_grid in range(0, 24, 3):
    ra_r = ra_grid * (2 * np.pi / 24)
    dec_vals = np.linspace(-60, 89, 100)
...
for s1_name, s2_name in edges:
    if s1_name in name_to_idx and s2_name in name_to_idx:
        ...
        p.line([proj_x[i1], proj_x[i2]], [proj_y[i1], proj_y[i2]], line_color="#4a6fa5", line_alpha=0.45, line_width=2)

# Plot stars
source = ColumnDataSource(data={"x": proj_x, "y": proj_y, "size": sizes, "color": star_colors, "name": star_names})

p.scatter(x="x", y="y", size="size", color="color", alpha=0.9, line_color=None, source=source)

# Label constellation names at centroids
...
# Label bright named stars (mag < 1.5)
bright_names_x = []
bright_names_y = []
bright_names_text = []
...
bright_source = ColumnDataSource(data={"x": bright_names_x, "y": bright_names_y, "text": bright_names_text})
