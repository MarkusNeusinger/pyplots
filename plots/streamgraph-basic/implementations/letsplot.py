...
# Generate smooth trends for each genre
raw_values = {}
for i, genre in enumerate(genres):
    base = 100 + 50 * np.sin(np.linspace(0, 4 * np.pi, n_months) + i * 0.5)
    trend = np.linspace(0, 30, n_months) * (1 if i % 2 == 0 else -0.5)
    noise = np.random.randn(n_months) * 10
    raw_values[genre] = np.clip(base + trend + noise, 20, None)

# Compute streamgraph positions (centered around baseline)
# Stack values and compute symmetric baseline offset
...
plot = (
    ...
    + geom_ribbon(aes(ymin="ymin", ymax="ymax"), alpha=0.85)  # noqa: F405
    ...
    + labs(  # noqa: F405
        x="Month", y="", fill="Genre", title="streamgraph-basic · lets-plot · pyplots.ai"
    )
